...
expressServer.post('/moment/create', (req, res) => {
    ...
    //.replace(':', '-') + '-' + moment.title.replaceAll(/[,.?'!@#$%^&()[*]/g, '');
    // moment.id = moment.time.replace(':', '-') + '-' + moment.title.replaceAll(/[,.?'!@#$%^&()[*]/g, '');
    // moment.id = moment.id.trim().replaceAll(' ', '-').toLowerCase()
    // moment.id = moment.id.replaceAll('--', '-').toLowerCase()
