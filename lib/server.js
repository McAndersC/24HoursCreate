...
expressServer.post('/moment/create', (req, res) => {
    ...
    let moment = req.body;
    moment.id = moment.time.replace(/:/g, '-') + '-' + moment.title.replace(/[,.?'!@#$%^&()[*]/g, '');
    moment.id = moment.id.toString().trim().replace(/ /g, '-').toLowerCase()
    moment.id = moment.id.toString().replace(/--/g, '-').toLowerCase()
    

    let folderName = moment.id;
    moment.url = '/moments/' + encodeURI(folderName) + '.html';
...
// Moment Create.
expressServer.get('/moment/pages/create', (req, res) => {
    ...
    res.setHeader('content-type', 'application/json');
    res.status(201).send({'message' : 'Created pages from full moment set.'});
});
